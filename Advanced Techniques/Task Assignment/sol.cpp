...
int main() {
    ...
    for(int i = 1; i <= n; i++) mcmf.AddEdge(i + n, 2 * n + 1, 1, 0);
    auto [c, f] = mcmf.Mincost(s,t);
    assert(f == n);
    cout << c << '\n';
    for(auto el : mcmf.edges) {
        if(el.flow && el.from >= 1 && el.from <= n && el.to > n && el.to <= 2 * n) cout << el.from << ' ' << el.to - n << '\n';
    }
    return 0;
}